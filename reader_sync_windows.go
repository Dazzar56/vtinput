...
func (r *ReaderSync) platformInit(in io.Reader) {
	useConPTY := true
	switch InputMode {
	case "ConPTY":
		useConPTY = true
		...
		if f, ok := in.(*os.File); ok {
			...
			if err := windows.GetConsoleMode(handle, &mode); err == nil {
				...
				if err := windows.SetConsoleMode(handle, newMode); err != nil {
					Log("ReaderSync: ConPTY SetConsoleMode failure: %v", err)
				}

				event, err := windows.CreateEvent(nil, 0, 0, nil)
				if err != nil {
					Log("ReaderSync: CreateEvent failed: %v", err)
				} else {
...
func (r *ReaderSync) readBytes(buf []byte, timeout time.Duration) (int, error) {
	if timeout > 0 {
...
func (r *ReaderSync) readConPTYEventTimeout(timeout time.Duration) (*InputEvent, error) {
	if r.conHandle == 0 {
...
func (r *ReaderSync) platformClose() {
	if r.cancelEvent != 0 {
